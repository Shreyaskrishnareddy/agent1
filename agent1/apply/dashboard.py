...
@dataclass
class WorkerState:
    """Tracks the current state of an apply worker."""
...
def render_dashboard() -> Table:
    ...
    table = Table(title="Agent1 Dashboard", expand=True, show_lines=False)
    ...
    for s in states:
        elapsed = ""
        if s.start_time and s.status in ("applying", "filtering"):
    ...
    table.add_row(
        "", "", "", "", "", "TOTAL",
        str(total_applied), str(total_failed),
        style="bold",
    )
